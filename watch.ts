...
const setupPreloadWatcher = async (viteServer: ViteDevServer) =>
  getWatcher('reload-app-on-preload-package-change', 'app/preload/vite.config.ts', () => {
...
const setupMainWatcher = async () => {
  const logger = createLogger('info', {prefix: '[back]'})
  let spawnProcess: ChildProcessWithoutNullStreams | null = null

  return getWatcher('reload-app-on-back-package-change', 'app/back/vite.config.ts', () => {
...
(async () => {
  try {
    const rendererServer = await createServer({
      ...sharedConfig,
      configFile: 'app/front/vite.config.ts',
    })
